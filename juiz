...
version,url,config,jmodule,loaded,hook,hooks,juiz,util = "Juiz IRC Bot 1.1.0","http://github.com/thelinx/Juiz",{},{},{},{},{},{},{}
...
for _,v in pairs(arg) do
  ...
  --display-raw-data - activates printing of raw irc data received.
...
function util.msg(mtype, mtext, ...)
    ...
    elseif DISPLAYRAWDATA and mtype == "RAW" then
        text = string.format("RAW DATA: %s", string.format(mtext, ...))
    end
    if text ~= "" then
        print(text)
    end
...
function juiz.addhook(trigger, func)
    ...
    return table.insert(hooks, {trigger, func})
end
    ...
    elseif command:lower() == "privmsg" then
        if recp:lower() == config.nick:lower() and onick:lower() ~= config.nick:lower() then
            if param:match("[^%w]?VERSION[^%w]?") then
                ...
                util.msg("NOTIFY", "Version requested from %s", onick)
                return true
            elseif param:match("[^%w]?PING[^%w]?") then
                juiz.send(string.format("PONG %s", ohost))
                util.msg("NOTIFY", "Ping requested from %s", onick)
                return true
            end
        end
...
local function mainloop()
    ...
    if not rdata and rerror and #rerror > 0 and rerror ~= "timeout" then
        util.msg("NOTIFY", "Lost connection to %s:%d: %s", config.server, config.port, rerror)
        error()
        return false
