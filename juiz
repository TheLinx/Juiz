...
version,url,config,jmodule,loaded,hook,hooks,juiz,util = "Juiz IRC Bot 1.2.0","http://github.com/thelinx/Juiz",{},{},{},{},{},{},{}
...
function juiz.callhook(trigger, ...)
    for _,v in pairs(hooks) do
        if v[1] == trigger then
            ...
            util.msg("TRACE", "hook to '%s' returned %s, err = %s", trigger, tostring(cret), cerr or "nil")
        end
    end
end
...
-- Add additional configuration items after this line, refer to individual module documentation for more information.]])
    fopn:close()
    util.msg("NOTIFY", "Please edit the configuration file created in %s.", CUSTOMCONFIG or "the current directory")
    alive = false
else
    fopn:close()
    dofile(configfile)
